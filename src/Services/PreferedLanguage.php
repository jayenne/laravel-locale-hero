...
namespace Jayenne\LaravelLocaleHero\Services;
...
class PreferedLanguage
{
    ...
    public function __construct($prefered_languages)
    {
        ...
        $this->fallback = config('locale-hero.fallback');
        $this->allowed = collect(config('locale-hero.allowed'));
        $this->client_prefered = $this->clientPreferedLanguages();
        $this->locale_code = $this->getLocaleHero();
    ...
    /**
     * It will find the first best match for locale_code according to the allowed list (from config file).
     *
     * @return \Illuminate\Config\Repository|int|mixed|string|static
     */
    protected function getLocaleHero()
    {
        $prefered = $this->rewritePreferedToFourDigitValues();

        // Find exact match for 4 digits
        $prefered = $this->client_prefered->keys()->filter(
            function ($value) {
        ...
        // Find first two digit (lang) match to four digit locale_code from the allowed-list
        if (null === $prefered) {
            $prefered = $this->client_prefered->keys()->filter(
                function ($value) {
    ...
    {
        $prefered = $this->client_prefered->keys()->map(
            function ($value) {
                if (5 == strlen($value)) {
                    return $value;
                } else {
    ...
    {
        $path = resource_path('/lang/').$this->locale_code.'.json';

        if (file_exists($path)) {
            return $this->locale_code;
        } else {
            return substr($this->locale_code, 0, 2);
        }
    }
